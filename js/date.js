	    ...
	    link: function link(scope, element, attrs, controller) {
	      ...
	      var initDateWidget = function initDateWidget() {
	        ...
	        function setVal(forcedUpdate) {
	          ...
	          var _viewValue = controller.$viewValue;
	          controller.$setViewValue(newViewValue);

	          // devansh
          	  _viewValue = _viewValue || !!_viewValue;        // to make null !== undefined
          	  newViewValue = newViewValue || !!newViewValue;  // true

			  if(!controller.changeCount){
	          	controller.changeCount = 0;
	          }
	          controller.changeCount++;

	          if(controller.changeCount == 1){ // first change is for setting initial value, not by user
	          	controller.$dirty = false;
	          }else{
	          	if( _viewValue !== newViewValue ){ //only if actually changed
